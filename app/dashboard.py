...
/* ── header ── */
.hdr { display: flex; align-items: center; justify-content: space-between; padding: 12px 20px; border-bottom: 1px solid var(--border); background: var(--surface); height: 48px; }
...
<div class="main">
  ...
  <div class="sidebar">
    ...
    <div class="tier-btn active" id="tb-all" onclick="setTier(null)">
      <div class="tier-left">
        <div class="dot dot-all"></div>
        <div><div class="tier-name">All</div></div>
      </div>
    ...
    <div class="tier-btn" id="tb-working" onclick="setTier('working')">
      <div class="tier-left">
        <div class="dot dot-working"></div>
        <div><div class="tier-name">Working</div><div class="tier-sub">L1 · max 1 h</div></div>
      </div>
    ...
    <div class="tier-btn" id="tb-episodic" onclick="setTier('episodic')">
      <div class="tier-left">
        <div class="dot dot-episodic"></div>
        <div><div class="tier-name">Episodic</div><div class="tier-sub">L2 · max 24 h</div></div>
      </div>
    ...
    <div class="tier-btn" id="tb-semantic" onclick="setTier('semantic')">
      <div class="tier-left">
        <div class="dot dot-semantic"></div>
        <div><div class="tier-name">Semantic</div><div class="tier-sub">RAM · max 7 d</div></div>
      </div>
    ...
    <div class="tier-btn" id="tb-archived" onclick="setTier('archived')">
      <div class="tier-left">
        <div class="dot dot-archived"></div>
        <div><div class="tier-name">Archived</div><div class="tier-sub">Disk · unlimited</div></div>
      </div>
      <span class="tier-count" id="cnt-archived">—</span>
    </div>

    <div class="sec-label">GC Pressure</div>
...
window.onload = () => { loadStats(); loadMems(); };
...
async function loadStats() {
  try {
    ...
    for (const t of ['working','episodic','semantic','archived']) {
      ...
      // update tier-sub with live token count
      const sub = document.querySelector('#tb-'+t+' .tier-sub');
      if (sub && d.count) sub.textContent = sub.textContent.split('·').slice(0,2).join('·') + ' · ' + fmt(d.total_tokens||0) + ' tok';
    }
...
function onSearch() {
  ...
  searchTmr = setTimeout(async () => {
    try {
      const url = '/memories/search?q=' + encodeURIComponent(q) + '&limit=50' + (activeTier ? '&memory_type='+activeTier : '');
...
function setTier(t) {
  ...
  document.getElementById('tb-'+(t||'all')).classList.add('active');
  const q = $('search').value.trim();
  q ? onSearch() : loadMems();
}
...
async function loadGCPreview() {
  ...
  try {
    ...
    const rows = [
      ...p.to_delete.map(e  => [e,'delete']),
      ...p.to_archive.map(e => [e,'archive']),
      ...p.to_demote.map(e  => [e,'demote']),
      ...p.to_promote.map(e => [e,'promote']),
      ...p.to_keep.map(e    => [e,'keep']),
    ];
    $('gc-inner').innerHTML = `<div class="gc-summary">${esc(p.summary)}</div>`
      + rows.slice(0,50).map(([e,a]) => `
        <div class="gc-row">
          <span class="ab ab-${a}">${a}</span>
          <span class="gc-text">${esc(e.content_preview||'')}</span>
          <span class="gc-score-val">${(e.score||0).toFixed(2)}</span>
        </div>`).join('');
